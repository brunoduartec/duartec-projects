...
function ms_site_check() {
	...
	if ( '1' == $current_blog->deleted ) {
		if ( file_exists( WP_CONTENT_DIR . '/blog-deleted.php' ) ) {
			...
			wp_die( /*WP_I18N_USER_DELETED_BLOG*/'Este usuário escolheu excluir sua conta e o conteúdo não está mais disponível.'/*/WP_I18N_USER_DELETED_BLOG*/ );
		}
	}
	...
	if ( '2' == $current_blog->deleted ) {
		if ( file_exists( WP_CONTENT_DIR . '/blog-inactive.php' ) )
			...
			wp_die( sprintf( /*WP_I18N_BLOG_NOT_ACTIVATED*/'O site não foi ativado ainda. Se você estiver com problemas para ativar seu site, contate <a href="mailto:%1$s">%1$s</a>.'/*/WP_I18N_BLOG_NOT_ACTIVATED*/, str_replace( '@', ' AT ', get_site_option( 'admin_email', "support@{$current_site->domain}" ) ) ) );
	}
	...
	if ( $current_blog->archived == '1' || $current_blog->spam == '1' ) {
		if ( file_exists( WP_CONTENT_DIR . '/blog-suspended.php' ) ) {
			...
			header( 'HTTP/1.1 410 Gone' );
			wp_die( /*WP_I18N_ARCHIVED*/'Este site foi arquivado ou suspenso.'/*/WP_I18N_ARCHIVED*/ );
		}
	}
...
function ms_not_installed() {
	...
	if ( ! is_admin() )
		die( $msg );
	$msg .= '<p>' . /*WP_I18N_CONTACT_OWNER*/'Se o seu site não for exibido, contate o dono desta rede.'/*/WP_I18N_CONTACT_OWNER*/ . '';
	$msg .= ' ' . /*WP_I18N_CHECK_MYSQL*/'Se você é o dono desta rede, verifique se o MySQL está funcionando adequadamente e se todas as tabelas estão livres de erros.'/*/WP_I18N_CHECK_MYSQL*/ . '</p>';
