...
?><small><a href="admin.php?page=Documentation#sect-1">Section 1 Help</a></small></legend>		
   <table class="optiontable"> 
    ...
    $email=get_option('gb_email_address');
if($email=="youremail@yourdomain.com" || $email==""){
    echo '<span class="alert">Your email address(<span class="abbr" title="M&uacute;ltiplos endere&ccedil;os devem ser separados por v&iacute;rgula.">es</span>)</span>: ';
} else {
     echo 'Your email address(<span class="abbr" title="M&uacute;ltiplos endere&ccedil;os devem ser separados por v&iacute;rgula.">es</span>): ';
} 
?></th> 
      <td><input type="text" value="<?php echo $options->gb_email_address; ?>" name="email" id="email" />
<?php 
if(strpos($email,".") === FALSE || strpos($email,"@") === FALSE) {
     echo ' <span class="reg_alert">&laquo; Este formato n&atilde;o &eacute; de um e-mail.</span>';
} 
?></td> 
    </tr>
    <tr>
...
if($gbcc == "gbcc") {
     $gb_cc = ", $email";
     $cc_notify1 = "".$x_or_h_br."<small>(Foi enviada uma c&oacute;pia para o seu email.)</small>";
     $cc_notify2 = "(C&oacute;pia enviada.)";
     $cc_notify3 = "";
} else {
     $gb_cc = "";
     $cc_notify1 = ""; 
...
if(!isset($gbname,$email,$reason,$message,$spamq) || empty($gbname) || empty($email) || empty($reason) || empty($message) || empty($spamq)){
     $forms.=('   <h'.$gb_heading.' class="formhead" id="results">Resultado: <span class="error">'.$error_heading.'</span></h'.$gb_heading.'> 
     <p><span class="error">Campos obrigat&oacute;rios n&atilde;o preenchidos:</span> Os seguintes campos n&atilde;o foram preenchidos. Por favor retorne e complete o formul&aacute;rio.</p>'."\n");
     $forms.=('      <dl>'."\n");
     $forms.=('       <dt>Campo(s) vazio(s):</dt>'."\n");
if(empty($gbname)) { 
     $forms.=('        <dd>&#8220;Digite seu nome completo&#8221;</dd>'."\n"); 
}
if(empty($email)) { 
     $forms.=('        <dd>&#8220;Digite seu e-mail&#8221;</dd>'."\n"); 
}
if(empty($reason)) { 
     $forms.=('        <dd>&#8220;Selecione um motivo para o contato&#8221;</dd>'."\n"); 
}
if(empty($message)) { 
     $forms.=('        <dd>&#8220;Escreva sua mensagem&#8221;</dd>'."\n"); 
}
if(empty($spamq)) { 
...
if(!ereg("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,6})", "$email")) {
     $forms.=('   <h'.$gb_heading.' class="formhead" id="results">Resultado: <span class="error">'.$error_heading.'</span></h'.$gb_heading.'>
     <p><span class="error">O e-mail parece inv&aacute;lido:</span> Por favor, preencha corretamente. </p>'."\n");
		 inc_spam_count();
// Anti-spam trap 1
} else if($trap1 !== "") {
     $forms.=('   <h'.$gb_heading.' class="formhead" id="results">Resultado: <span class="error">'.$error_heading.'</span></h'.$gb_heading.'>
     <p><span class="error">Anti-Spam Trap 1 Field Populated:</span> Parece que que voc&ecirc; &eacute; um spam. Cai fora!</p>'."\n");
     inc_spam_count();
// Anti-spam trap 2
} else if($trap2 !== "") {
     $forms.=('   <h'.$gb_heading.' class="formhead" id="results">Resultado: <span class="error">'.$error_heading.'</span></h'.$gb_heading.'>
     <p><span class="error">Parece que que voc&ecirc; &eacute; um spam.</span> Cai fora.</p>'."\n");
		 inc_spam_count();
// Input length error tripping
} else if(strlen($gbname) > 40 || strlen($email) > 40 || strlen($phone) > 30 || strlen($url) > 60 || strlen($gbcc) > 4) {
       $forms.=('   <h'.$gb_heading.' class="formhead" id="results">Resultado: <span class="error">'.$error_heading.'</span></h'.$gb_heading.'>
     <p><span class="error">Muitas palavras: </span>Parece que que voc&ecirc; &eacute; um spam. Cai fora!</p>'."\n");
		 inc_spam_count();
// Contact reason validation
} else if(!in_array($reason, $gb_options)) { 
       $forms.=('   <h'.$gb_heading.' class="formhead" id="results">Resultado: <span class="error">'.$error_heading.'</span></h'.$gb_heading.'>
     <p><span class="error">Motivo inexistente:</span> Parece que que voc&ecirc; &eacute; um spam. Cai fora!</p>'."\n");
		 inc_spam_count();
// Check the IP black list
} else if(in_array($ip, $ip_blacklist)) { 
       $forms.=('   <h'.$gb_heading.' class="formhead" id="results">Resultado: <span class="error">'.$error_heading.'</span></h'.$gb_heading.'>
     <p><span class="error">IP Negro:</span> Parece que que voc&ecirc; &eacute; um spam. Cai fora!.</p>'."\n");
		 inc_spam_count();
// Form value confirmation
} else if($formid !== "GB".$form_id."") {
     $forms.=('   <h'.$gb_heading.' class="formhead" id="results">Resultado: <span class="error">'.$error_heading.'</span></h'.$gb_heading.'>
     <p><span class="error">ID estranho:</span> Parece que que voc&ecirc; &eacute; um spam. Cai fora!</p>'."\n");
		 inc_spam_count();
// My long version of Jem's exploit killer
     ...
     $forms.=('   <h'.$gb_heading.' class="formhead" id="results">Resultado: <span class="error">'.$error_heading.'</span></h'.$gb_heading.'>
     <p><span class="error">Muito estranho:</span> Parece que que voc&ecirc; &eacute; um spam. Cai fora!</p>'."\n");
		 inc_spam_count();
// Anti-spam verification
} else if($spamq !== "$gb_randoma") {
     $forms.=('   <h'.$gb_heading.' class="formhead" id="results">Resultado: <span class="error">'.$error_heading.'</span></h'.$gb_heading.'>
     <p><span class="error">Resposta anti-spam errada:</span> Resposta errada. Se continuar com dificuldades pode tentar '.$my_or_our.' email regular , <a href="mailto:'.$gb_email_address.'?subject='.$gb_website_name.'%20Backup%20Email%20[Resposta anti-spam errada]">'.$gb_email_address.'</a>. Obrigado e desculpe o transtorno.</p>'."\n");
		 inc_spam_count();
// Let match the referrer to ensure it's sent from here and not elsewhere
} else if($new_referrer !== $form_location) {
     $forms.=('   <h'.$gb_heading.' class="formhead" id="results">Resultado: <span class="error">'.$error_heading.'</span></h'.$gb_heading.'>
     <p><span class="error">Erro:</span> Algo deu errado, tente '.$my_or_our.' email regular, <a href="mailto:'.$gb_email_address.'?subject='.$gb_website_name.'%20Backup%20Email%20[Dado errado]">'.$gb_email_address.'</a>. Desculpe o transtorno.</p>'."\n");
		 inc_spam_count();
// And now let's see if the variable for submit matches what's required
} else if(!(isset($_POST[''.$send_value.'']))) {
     $forms.=('   <h'.$gb_heading.' class="formhead" id="results">Resultado: <span class="error">'.$error_heading.'</span></h'.$gb_heading.'>
     <p><span class="error">Erro de envio:</span> Algo deu errado, tente '.$my_or_our.' email regular, <a href="mailto:'.$gb_email_address.'?subject='.$gb_website_name.'%20Backup%20Email%20[Dado errado]">'.$gb_email_address.'</a>. Desculpe o transtorno.</p>'."\n");
		 inc_spam_count();
// Holy smokes, looks like all's cool and we can send the message
} else {
     $gb_content = "Oi, $gb_contact_name,\n\nContato enviado via $gb_website_name por $gbname. Ele escreveu os seguintes dados:\n\n   E-mail: $email $cc_notify2\n   Telefone: $phone\n   Website: $url\n   Motivo: $reason\n\nMensagem:\n   $message\n\n\n--------------------------\nOutros dados:\n   IP: $ip\n   Tempo: $ltd\n   De onde veio: $hr\n   Host: $hst\n   User Agent: $ua\n   Resolve IP Whois: http://www.arin.net/whois/\n\n";
     $gb_ccmail = "Oi $gbname,\n\nDuplicata Carbono do email que enviou para $gb_website_name. Responderemos em breve. Seus dados:\n\n   Email: $email $cc_notify3\n   Telefone: $phone\n   Website: $url\n   Motivo: $reason\n\nMensagem:\n   $message\n\n\n--------------------------\nMais dados:\n   Tempo: $ltd\n\n";
...
// The mail function helps, let's send this stuff
     mail("$gb_email_address", "[$gb_website_name] Contato de $gbname", $gb_content, $gb_email_header);

if($gb_cc !== "") {
     mail("$gb_cc", "[Duplicata Carbono] Email enviado para $gb_website_name", $gb_ccmail, $gb_email_header);
}

// And let's inform the user and show them what they sent
     $forms.=('   <h'.$gb_heading.' class="formhead" id="results">Resultado: <span class="success">'.$success_heading.'</span> <small>[ <a href="'.$hr.'">Limpar formul&aacute;rio</a> ]</small></h'.$gb_heading.'>
    <p><span class="success">Mensagem enviada </span>com sucesso, '.$i_or_we.' responderemos em breve. Voc&ecirc; enviou os seguintes dados:</p> 
     <ul>
      <li><span class="items">Nome:</span> '.$gbname.'</li>
      <li><span class="items">Email:</span> <a href="mailto:'.$email.'">'.$email.'</a> '.$cc_notify1.'</li>
      <li><span class="items">Telefone:</span> '.$phone.'</li>
      <li><span class="items">Website:</span> <a href="'.$url.'">'.$url.'</a></li>
      <li><span class="items">Motivo:</span> '.$reason.'</li>
     </ul>
    <dl id="result_dl_blockq">
     ...
     <dl>
      <dt><small>Tempo:</small></dt>
       <dd><small>'.$ltd.'</small></dd>
     </dl>'."\n");
  }
 }
} else { 
// No errors so far? No successes so far? No confirmation? Hmm. Maybe the user needs a contact form
 if ($options->showformhead=='yes'){
     $forms.=('  <h'.$gb_heading.' class="main_formhead"></h'.$gb_heading.'>'."\n"); 
 }
     $forms.=('  <form id="gb_form" method="post" action="'.htmlentities($_SERVER['REQUEST_URI']).'#results">
<!-- Form Intro -->
   <fieldset id="formwrap">
      <legend id="mainlegend" style="cursor:help;" title="Nem pense em colar html por aqui.!">');
...
<!-- Required Info -->
      <fieldset>
       <legend>Seus dados:</legend>
        <label for="name">Nome completo'.$x_or_h_br.'<input tabindex="'.$tab_name.'" class="med" type="text" name="gbname" id="name" size="35" maxlength="40" value=""'.$x_or_h_in.'></label>'.$x_or_h_br.' 
        <label for="email">Seu e-mail'.$x_or_h_br.'<input tabindex="'.$tab_email.'" class="med" type="text" name="email" id="email" size="35" maxlength="40" value=""'.$x_or_h_in.'></label>
      </fieldset>
<!-- Optional Info -->
      <fieldset>
       <legend>Complemento (opcional):</legend>
        <label for="phone">Telefone'.$x_or_h_br.'<input tabindex="'.$tab_phone.'" class="med" type="text" name="phone" id="phone" size="35" maxlength="30" value=""'.$x_or_h_in.'></label>'.$x_or_h_br.' 
        <label for="url">Endere&ccedil;o do seu website'.$x_or_h_br.'<input tabindex="'.$tab_url.'" class="med" type="text" name="url" id="url" size="35" maxlength="60" value="http://"'.$x_or_h_in.'></label>
      </fieldset>
<!-- Required Reasons -->
      <fieldset>
       <legend>Escolha</legend>
        <label for="reason">o motivo de seu contato:'.$x_or_h_br.' 
         <select tabindex="'.$tab_reason.'" class="med" style="cursor:pointer;" name="reason" id="reason">
          <option value="" selected="selected">Escolha uma das op&ccedil;&otilde;es</option>'."\n"); 
reset($gb_options);
...
<!-- Required Form Comments Area -->
      <fieldset>
       <legend>Escreva</legend>
        <label for="message">sua mensagem:'.$x_or_h_br.'<textarea tabindex="'.$tab_message.'" class="textbox" rows="12" cols="60" name="message" id="message"></textarea></label>
      </fieldset>
<!-- Required anti spam confirmation -->
      <fieldset>
       <legend>Anti-spam:</legend>
        <label title="N&atilde;o se preocupe com mai&uacute;sculas ou min&uacute;sculas." for="spamq">'.$gb_randomq.' <input tabindex="'.$tab_spam.'" class="short" type="text" name="spamq" id="spamq" size="15" maxlength="30" value=""'.$x_or_h_in.'> <small class="whythis" title="Isto confirma que voc&ecirc; &eacute; um ser humano.">- <a tabindex="'.$tab_why.'" href="#spamq" style="cursor:help;">Porque isso? </a></small></label>'.$x_or_h_br.' 
...
<!-- Special anti-spam input: non-displayed type -->
       <div style="position:absolute; top: -9000px; left:-9000px;">'.$x_or_h_br.' 
        ...
        <input type="text" name="p-mail" id="p-mail" alt="Anti-Spam Trap Two" value=""'.$x_or_h_in.'>
        </label>
       </div>
<!-- Special anti-spam form id field -->
        <input type="hidden" name="GB'.$form_id.'" id="GB'.$form_id.'" alt="Campo de ID" value="GB'.$form_id.'"'.$x_or_h_in.'>
      </fieldset>
<!-- Form Buttons -->
      <fieldset>
       <legend>Clique para enviar:</legend>'."\n");
if(@$gb_show_cc == "yes") {
    $forms.=('         <label for="gbcc"><input tabindex="'.$tab_cc.'" class="checkbox" type="checkbox" name="gbcc" id="gbcc" value="gbcc" /> <small> Marque aqui se deseja uma c&oacute;pia deste formul&aacute;rio em seu e-mail..</small></label>'.$x_or_h_br.''."\n"); 
} else {
    $forms.=(''."\n");
}
    $forms.=('         <input tabindex="'.$tab_submit.'" style="cursor:pointer;" class="button" type="submit" alt="Clique no bot&atilde;o para '.$send_button.'" value="'.$send_button.'" name="'.$send_value.'" id="'.$send_value.'" title="Clique no bot&atilde;o para enviar."'.$x_or_h_in.'>'."\n"); 
if(@$showcredit == "yes") {
