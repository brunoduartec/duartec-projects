...
, detector(d)
, extractor(e)
, matcher(matcherForDescriptorType(e->descriptorSize(), /*e->defaultNorm()*/  cv::NORM_L2, useBruteForceMather))
{
    CV_Assert(d);
    CV_Assert(e);
}
...
, knMatchSupported(false)
, featureEngine(fe)
, matcher(matcherForDescriptorType(fe->descriptorSize(),/* fe->defaultNorm()*/ cv::NORM_L2 , useBruteForceMather))
{
    CV_Assert(fe);
}
