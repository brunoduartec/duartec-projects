...
Como descrito na seção \ref{sec:analisededados}, existem restrições para
garantir que a técnica utilizada responde segundo as necessidades da aplicação.
...
\begin{axis}
	...
	\addplot[red,sharp plot,update limits=false] coordinates	{(\minx,\ylimit)(\maxx,\ylimit)};
	 \fill [opacity=0.4,red!25] (axis cs:\minx,\ylimit) rectangle(axis cs:\maxx,\miny);
						...
						%eixo vertical
	\addplot[blue,sharp plot,update limits=false] coordinates	{(\minbright,\miny)(\minbright,\maxy)} ; 
	\fill [opacity=0.4,blue!25] (axis	cs:\minx,\miny) rectangle (axis	cs:\minbright,\maxy);
	
	\addplot[blue,sharp plot,update limits=false] coordinates	{(\maxbright,\miny)(\maxbright,\maxy)} ; 
...
\hline
	...
	SCALE PRECISION &  &  & X & X &  & X &  &  &  \\ \hline
	ROTATION PRECISION & X & X & X & X & X & X & X & X & X \\ \hline
	BLUR PRECISION &  & X & X & X & X & X & X & X & X \\ \hline
	BRIGHT PRECISION & X & X & X & X & X & X & X & X & X \\ \hline
	SCALE TIME & X &  &  & X &  & X & X &  &  \\ \hline
	ROTATION TIME & X &  &  & X &  & X & X &  &  \\ \hline
	BLUR TIME & X &  &  & X &  & X & X &  &  \\ \hline
	BRIGHT TIME & X &  &  & X &  & X & X &  &  \\ \hline
\end{tabular}
}
