...
O descritor de características é bem parecido com o descrito em SIFT na
Seção~\ref{sec:sift}, sendo SURF\cite{SURF} desenvolvido para tentar suprir as
principais fraquesas de SIFT, alta complexidade computacional e  baixa velocidade de
execução.
A detecção de SURF utiliza aproximações baseadas no determinante da matriz
Hessiana equação~\ref{eq:hessiana}  mostrado na equação~\ref{eq:determinantehessiana}.
\begin{equation}
...
Aproximando as segundas derivadas indo muito mais além do que o abordado no
algoritmo de SIFT que aproxima LoG por Diferencas de Gaucianas(DoG), SURF vai um pouco mais além e aproxima LoG de \emph{Box-type Filter} como
...
\caption{Box Filtering. Fonte(\cite{SURF}) }
\label{fig:surf01}
\end{figure}


O descritor usa \emph{integral images} para interpretar a distribuição de
...
adotando-se uma orientação dominante para fazer o descritor invariante a
rotação.
...
\caption{Janela do Descritor, 20 vezes o tamanho do ponto determinado e
orientado de acordo com a direção domimante. Fonte(\cite{SURF}) }
...
direções. Cada sub região contribui para o descritor com quatro componentes: a
