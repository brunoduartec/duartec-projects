...
$oResult = mysql_query("SELECT distinct `enct_name`, `enct_email`  FROM `encontro` WHERE enct_dt_registration >='2014-05-11' ORDER BY `enct_name` ASC",$conexao);
...
//	$oResult = mysql_query("SELECT * from encontro where year(enct_dt_inscricao) = 2013 order by enct_nome", $conexao);
  ...
  {
	echo "<tr><td>" . $idx . "</td><td>" .$row['enct_name'] . "</td><td>" .$row['enct_email']. "</td></tr>";
	$idx++;  
}
